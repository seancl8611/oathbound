...
1. A skilled player can dash into a normal engagement, commit to an offensive string, and kill an Area 1 standard Swordsman in roughly **5-6 clean base-katana hits** if none of those hits are guarded.
2. A meaningful Swordsman guard can make that same offensive commitment fail to kill, forcing the player either to continue pressure or disengage/reposition.
3. Corrupted Archers are distinctly squishier than Swordsmen; the initial target is roughly **5 clean base-katana hits**.
4. The Warden remains the durable standard-enemy extreme; the initial target is roughly **10-11 clean base-katana hits**, before considering successful full-Health blocks.
5. Lightweight Hollow/Hound bodies remain faster to remove than the standard Swordsman and derive danger from numbers, movement, and pressure rather than individual durability.
...
The initial calibration anchor is the approved pre-awakening base-katana basic chain in `AspectCatalog.gd`:

- Quick Slash: **9 Health damage**
- Cross Cut: **12 Health damage**
- Heavy Cleave: **21 Health damage**
- one 3-hit chain: **42 Health damage**
- two uninterrupted chains / 6 hits: **84 Health damage**

This produces the first Area 1 targets:

| Enemy | Initial Health target | Clean base-katana expectation | Defensive interpretation |
|---|---:|---|---|
| Hollow | 45 | about 4 hits | no traditional guard; fodder danger comes from swarm pressure |
| Blighted Hound | 50 | about 4 hits | no traditional guard; predator pressure and mobility are the defense |
| Corrupted Archer | 60 | about 5 hits | weak/occasional reactive guard can sometimes extend the kill |
| Corrupted Swordsman | 80 | 6 hits | a meaningful guard should normally let it survive the otherwise lethal 6-hit line |
| Cellar Bilemass | 80 | about 6 hits | no guard; hazard pressure and committed spit Poise are the defense |
| Warden | 140 | about 11 hits | short full-Health guard can extend the fight; it remains the tankiest standard Area 1 body |
...
## Why Swordsman 80 is intentional

The clean six-hit base-katana line deals 84 Health damage, so an 80-Health Swordsman dies if the player lands everything cleanly. Its current authored guard passes only 35% of Health damage. Even guarding the lightest 9-damage hit removes enough damage from the six-hit line that the Swordsman normally survives. This is the intended Area 1 interaction:

**clean execution -> one offensive commitment can kill**  
**successful enemy defense -> the same commitment may not kill**

The player should therefore feel rewarded for speed and clean offense without guards becoming cosmetic.
...
- **Swordsman:** moderate tactical guard frequency. It should sometimes interrupt a clean one-combo kill line, but not make every approach become a blocking duel.
...
- Does a clean six-hit Swordsman kill feel satisfyingly fast rather than trivial?
- Do occasional guards create useful variation without returning combat to a duel/block waiting game?
- Does an Archer disappear quickly enough when Akio successfully closes distance?
...
- Do whole encounters feel appropriately substantial even when individual waves vary greatly in clear time?
