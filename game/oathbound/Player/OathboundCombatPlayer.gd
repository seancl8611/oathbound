...
func _flash_player(color: Color, duration: float) -> void:
	if typeof(SettingsManager) == TYPE_OBJECT and SettingsManager.has_method("should_reduce_flashing") and bool(SettingsManager.should_reduce_flashing()):
		var base := _base_sprite_modulate if typeof(_base_sprite_modulate) == TYPE_COLOR else Color.WHITE
		super._flash_player(base.lerp(color, 0.35), minf(duration, 0.06))
		return
...
func _apply_impact_vibration(intensity: float, duration: float) -> void:
	...
	if not SettingsManager.has_method("vibration_enabled") or not bool(SettingsManager.vibration_enabled()):
		return
	var strength := clampf(float(SettingsManager.get_vibration_strength()), 0.0, 1.0) if SettingsManager.has_method("get_vibration_strength") else 1.0
	if strength <= 0.001:
		return
	var normalized := clampf(intensity / maxf(1.0, SHAKE_HEAVY), 0.0, 1.0)
	var weak := normalized * 0.30 * strength
	var strong := normalized * 0.70 * strength
