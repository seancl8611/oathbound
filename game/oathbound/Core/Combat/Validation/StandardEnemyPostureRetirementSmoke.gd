...
func _verify_standard_enemy(enemy_type: String) -> void:
	...
	# Direct legacy receiver mutation must be neutralized synchronously before the
	# CombatController can evaluate a break.
	...
	# Canonical AttackEvent compatibility fallback must also remain inert.
	...
	# Legacy parry notifications are likewise unable to recreate the retired meter.
