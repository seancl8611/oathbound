...
func _ready() -> void:
	...
	prompt.text = "Press E to approach Shrine"
	_build_ui()
...
func _refresh_ui() -> void:
	...
	if runtime == null:
		_title.text = "Shrine unavailable"
		_state_label.text = "CorruptionRuntime is not available."
	...
	var aspect: String = str(AspectRuntime.selected_aspect).capitalize() if typeof(AspectRuntime) == TYPE_OBJECT else ""

	_title.text = "Shrine of Returning Blood" if awakened else "Silent Shrine"
	_current_label.visible = awakened
	_next_label.visible = awakened

	if awakened:
		_state_label.text = "%s — Tier %d    |    Corruption %d / 100" % [aspect, tier, corruption]
		_current_label.text = "Current: %s" % str(runtime.call("get_current_tier_headline"))
		_next_label.text = "Next: %s" % str(runtime.call("get_next_tier_headline"))

	match shrine_state:
		"pre-awakening-support":
			_state_label.text = "The Shrine answers quietly. Something deeper has not yet awakened."
			_detail_label.text = "Restore 20% maximum Health and 25% maximum Spirit. Each resource restores independently."
			_set_action_visibility(true, false, false, false)
			_support_button.text = "Receive Support — +20% Health / +25% Spirit"
		"support":
			_detail_label.text = "Corruption is not full. This Shrine provides fixed support without changing Corruption."
			_set_action_visibility(true, false, false, false)
			_support_button.text = "Receive Support — +20% Health / +25% Spirit"
		"full-choice":
			_detail_label.text = "Corruption is full. Resist keeps this Tier and recovers resources; Embrace advances exactly one Aspect Tier."
			_set_action_visibility(false, true, true, false)
			_resist_button.text = "Resist — Corruption 75 / 100, +25% Health, +35% Spirit"
			_embrace_button.text = "Embrace — Advance to Tier %d, Corruption 0 / 100" % mini(4, tier + 1)
		"stabilize":
			_detail_label.text = "Tier IV is maximum. Stabilize releases excess pressure without granting additional Aspect power."
			_set_action_visibility(false, false, false, true)
			_stabilize_button.text = "Stabilize — Corruption 50 / 100, +30% Health, +40% Spirit"
		_:
			_detail_label.text = "Shrine state unavailable."
			_set_action_visibility(false, false, false, false)
