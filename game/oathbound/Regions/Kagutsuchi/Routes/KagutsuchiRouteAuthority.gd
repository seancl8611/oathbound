...
## Canonical Region 3 route authority.
## Implements the approved 11-counted-chamber Kagutsuchi Court route while retaining
## the existing RouteGenerator choice/gate plumbing used by GameFlow.

const TOTAL_CHAMBERS: int = 11
const BOSS_CHAMBER: int = 11
...
const ROOM_WEIGHTS: Dictionary = {
	...
	"final": {"combat": 58.0, "shrine": 5.0, "rest": 13.0, "merchant": 13.0, "treasure": 11.0},
}

# Current RewardPickup directly resolves these categories. Approved recovery,
# temporary-capacity, and reroll weights remain capability-gated and are
# proportionally redistributed until their unified route-reward bridge is current.
...
const FINAL_PREP: Array[String] = ["rest", "merchant", "combat:technique", "treasure"]
...
static func generate(route_generator: Node) -> Array[String]:
	...
	for chamber_number: int in range(1, BOSS_CHAMBER):
		...
		# Branching begins immediately. Guaranteed opportunities remain optional choices.
...
static func _build_guarantee_slots(route_generator: Node) -> Dictionary:
	# Separate Shrine / Merchant / Rest so safe-service pressure never becomes an
	# accidental service corridor. Main-build opportunities stay before the final
	# approach, while final preparation is explicitly reserved for Chamber 9 or 10.
	var service_candidates: Array[int] = [1, 2, 3, 4, 5, 6, 7, 8]
	...
	var technique_candidates: Array[int] = [1, 2, 3, 4, 5, 6, 7, 8]
	...
	var final_prep_slot: int = 9 if route_generator.rng.randi_range(0, 1) == 0 else 10
...
static func _forced_tokens(route_generator: Node, chamber_number: int, guarantees: Dictionary) -> Array[String]:
	...
	if chamber_number == int(guarantees.get("final_prep", -1)):
		var prep: String = FINAL_PREP[route_generator.rng.randi_range(0, FINAL_PREP.size() - 1)]
...
static func _validate(route_generator: Node) -> void:
	if route_generator.current_route.size() != TOTAL_CHAMBERS:
		push_error("[KagutsuchiRouteAuthority] generated %d chambers; expected %d" % [route_generator.current_route.size(), TOTAL_CHAMBERS])
	if route_generator.current_route.is_empty() or not str(route_generator.current_route[0]).begins_with("CHOICE_"):
		push_error("[KagutsuchiRouteAuthority] Chamber 1 must begin with a route choice")
	if route_generator.get_base_room_type(str(route_generator.current_route[BOSS_CHAMBER - 1])) != "boss":
		push_error("[KagutsuchiRouteAuthority] Chamber 11 must be Eclipse Shogun/boss")
