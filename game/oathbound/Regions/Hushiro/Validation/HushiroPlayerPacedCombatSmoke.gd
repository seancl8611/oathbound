...
## Protects hack-and-slash kill-time calibration, guard significance, wave-advance timing,
## arrival-mode variety, and encounter-level wave-count/population variability.
...
func _validate_base_katana_kill_time_contract() -> void:
	...
	var damage_2: int = _basic_damage_after_hits(profiles, 2)
	var damage_3: int = _basic_damage_after_hits(profiles, 3)
	...
	_expect(damage_2 == 21 and damage_3 == 42 and damage_4 == 51, "Base katana 2/3/4-hit cumulative damage must remain 21/42/51")
	...
	var bilemass_hp: int = _health_for(baselines, "bilemass")
	var warden_hp: int = _health_for(baselines, "warden")

	_expect(hollow_hp > damage_2 and hollow_hp <= damage_3, "Hollow fodder must die in about three clean base hits")
	_expect(hound_hp > damage_3 and hound_hp <= damage_4, "Hound must die in about four clean base hits")
	_expect(archer_hp > damage_3 and archer_hp <= damage_4, "Archer must be squishy and die by clean hit four")
	_expect(swordsman_hp > damage_4 and swordsman_hp <= damage_5, "Standard Swordsman must survive four clean base hits and die by hit five")
	_expect(bilemass_hp > damage_4 and bilemass_hp <= damage_5, "Bilemass must stay inside the normal five-hit kill envelope")
	_expect(warden_hp > damage_10 and warden_hp <= damage_11, "Warden must remain the deliberate 10-11-hit durable exception")
...
func _validate_guard_survival_contract() -> void:
	...
	if profiles.size() < 3:
		return
	var five_hit_damage: float = float(_basic_damage_after_hits(profiles, 5))
	...
	var guarded_five_hit_damage: float = five_hit_damage - first_hit + first_hit * swordsman_profile.guard_health_multiplier
	var swordsman_hp: float = float(_health_for(HUSHIRO_ENEMY_CONTRACT.BASELINES, "swordsman"))

	_expect(five_hit_damage >= swordsman_hp, "Five clean base hits must kill the Area 1 Swordsman")
	_expect(guarded_five_hit_damage < swordsman_hp, "One meaningful Swordsman guard must deny the otherwise lethal five-hit line")
	_expect(six_hit_damage > swordsman_hp, "Six-hit pressure extension must retain enough damage to finish a normal Swordsman after a defended five-hit line")
...
func _finish() -> void:
	if _failures.is_empty():
		print("[HushiroPlayerPacedCombatSmoke] PASS - 3-hit Hollow | 4-hit Hound/Archer | 5-hit Swordsman/Bilemass | 11-hit Warden | guard extends kill | immediate clear advance | 120s anti-stall | varied arrivals")
		get_tree().quit(0)
		return
